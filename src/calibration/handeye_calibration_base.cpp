#include "MSALIB/calibration/handeye_calibration_base.h"

namespace MSALIB {

HandEyeCalibrationBase::HandEyeCalibrationBase() {
 ...
 * @return 本组运动是否合格   
 */
bool HandEyeCalibrationBase::AddPose(const  Pose& pose_primary, const  Pose& pose_sub) {      
    // 首先检查pose 
 ...
 * @return 是否成功
 */            
bool HandEyeCalibrationBase::CalibExRotation() {
...
bool HandEyeCalibrationBase::CalibExTranslation() {
...
bool HandEyeCalibrationBase::calibExTranslationNonPlanar() {
...
bool HandEyeCalibrationBase::GetCalibResult(Eigen::Isometry3d &result) {
    if (calib_done_) {
 ...
 * @return {*}
 */            
bool HandEyeCalibrationBase::checkScrewMotion(const Pose &pose_primary, const Pose &pose_sub) {
    // 先检查当前数据是否正确
    // 提取出数据的旋转部分  
    ...
    if ((r_dis > EPSILON_R) || (t_dis > EPSILON_T)) {
        if ((r_dis > EPSILON_R)) {
            std::cout << color::RED << "r_dis > EPSILON_R !!" << color::RESET<<std::endl;
        }
        if (t_dis > EPSILON_T) {
            std::cout<<color::RED<<"t_dis > EPSILON_T!!"<<std::endl;
        }
        primary_lidar_accum_pose_.SetIdentity();
        sub_lidar_accum_pose_.SetIdentity();  
        return false;
