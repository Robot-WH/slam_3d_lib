...
class IVox {
   public:
    ...
    int DeletePointsPar(const PointCloud& points_to_add);

    int DeletePointsPar(const PointVector& points_to_add);
...
//     // 如果近邻点不足  则去邻居voxel中搜索
//     if (num < max_num) {
//         std::queue<Eigen::Matrix<int, dim, 1>> seed;
//         seed.push(key);
//         while (!seed.empty()) {
//             auto& key = seed.front();  
//             // 遍历全部邻居
    ...
    std::for_each(std::execution::unseq, points_to_add.begin(), points_to_add.end(), 
        [this](const auto& pt) {
            ...
            if (iter == grids_map_.end()) {
                ...
                grids_cache_.push_front({key, NodeType(center, options_.resolution_)});  //  这是个list 
                grids_map_.insert({key, grids_cache_.begin()});   // 这是一个unordered_map

                grids_cache_.front().second.InsertPoint(pt);
                // 限制一个grid的数量    将list最末尾的元素删除，即删除最早创建的grid
                ...
                // 将该体素剪贴到链表头  最新使用过的调整到链表最前面，实际就是一个LRU缓存 
                //  函数调用的一般形式为：splice(position, source, it)
                // position: 表示目标位置，这里使用grids_cache_.begin()，即grids_cache_的开头位置。
                // source: 表示要从中移除元素的位置，这里同样是grids_cache_，因为我们要从grids_cache_中移除一个元素。
                // it: 表示要移除元素的迭代器，这里是iter->second。
 ...
 */
template <typename PointType, int dim, IVoxNodeType node_type>
int IVox<PointType, dim, node_type>::DeletePointsPar(const PointCloud& points_to_add) {
    std::atomic num{0}; 
    int N = 0; 
    // 遍历所有点  添加到体素中   
    std::for_each(std::execution::par, points_to_add.points.begin(), points_to_add.points.end(), 
        [&](const auto& pt) {
            auto key = Pos2Grid(ToEigen<float, dim>(pt));  // grid地图中的三维坐标
            auto iter = grids_map_.find(key);
 ...
 */
template <typename PointType, int dim, IVoxNodeType node_type>
int IVox<PointType, dim, node_type>::DeletePointsPar(const PointVector& points_to_add) {
    std::atomic num{0};  
    int N = 0; 
    // 遍历所有点  添加到体素中   
    std::for_each(std::execution::par, points_to_add.begin(), points_to_add.end(), 
        [&](const auto& pt) {
            auto key = Pos2Grid(ToEigen<float, dim>(pt));  // grid地图中的三维坐标
            auto iter = grids_map_.find(key);
...
Eigen::Matrix<int, dim, 1> IVox<PointType, dim, node_type>::Pos2Grid(const IVox::PtType& pt) const {
    ...
    // return (pt * options_.inv_resolution_).array().floor().template cast<int>();  // floor()向下取整
}
