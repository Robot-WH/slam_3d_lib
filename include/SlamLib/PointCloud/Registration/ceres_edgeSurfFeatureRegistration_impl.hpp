...
CeresEdgeSurfFeatureRegistration<_PointType>::CeresEdgeSurfFeatureRegistration(std::string const& edge_label, std::string const& surf_label)
 ...
 */            
template<typename _PointType>
void CeresEdgeSurfFeatureRegistration<_PointType>::SetInputTarget(
 ...
 */        
template<typename _PointType>
void CeresEdgeSurfFeatureRegistration<_PointType>::SetInputSource(
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename _PointType>
void CeresEdgeSurfFeatureRegistration<_PointType>::SetMaxIteration(
        uint16_t const& n) {
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename _PointType>
void CeresEdgeSurfFeatureRegistration<_PointType>::SetNormIteration(uint16_t const& n) {
}
 ...
 */            
template<typename _PointType>
bool CeresEdgeSurfFeatureRegistration<_PointType>::Solve(Eigen::Isometry3d &T) {
    time::TicToc tt;  
    tt.tic();  
    // LOG(INFO) << "Solve";
    ...
    for (int iterCount = 0; iterCount < optimization_count_; iterCount++) {
        ...
        problem.AddParameterBlock(parameters, 7, new ceres_param::PoseSE3Parameterization());
...
CeresEdgeSurfFeatureRegistration<_PointType>::GetRegistrationResult() const {
...
template<typename _PointType>
std::vector<std::string> CeresEdgeSurfFeatureRegistration<_PointType>::GetUsedPointsName() {
...
// 加速后：约33ms 
template<typename _PointType>
void CeresEdgeSurfFeatureRegistration<_PointType>::addSurfCostFactor(
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), 
        [&](const size_t &i) {
            ...
            if (surf_match_.Match(point_temp, res)) {
                ...
                factor_container[i] = new ceres_factor::se3PointSurfFactor(ori_point, res.norm_, res.D_);   
 ...
 */        
template<typename _PointType>
void CeresEdgeSurfFeatureRegistration<_PointType>::addEdgeCostFactor(
        ceres::Problem& problem, ceres::LossFunction *loss_function,
        bool const& save_match_info) {
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const size_t &i) {
        ...
        if (edge_match_.Match(point_in_ref, res)) {
            ...
            factor_container[i] = new ceres_factor::se3PointEdgeFactor(ori_point, res.points_edge_[0], res.points_edge_[1]);    
        }
        // 保存该点的匹配残差信息，匹配点信息 
        if (save_match_info) {
            points_registration_res_[surf_label_].residuals_[i] = res.residuals_;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename _PointType>
void CeresEdgeSurfFeatureRegistration<_PointType>::pointAssociateToMap(
        _PointType const *const pi, _PointType *const po) {
